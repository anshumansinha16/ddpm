...
CONFIG['data_x_path'] = 'data_x/perov_5_raw_train_dist_mat.pt'
CONFIG['data_ead_path'] = 'data_x/perov_5_raw_train_ead_mat.pt'
CONFIG['composition_path'] = 'data_x/perov_5_raw_train_composition_mat.pt'
CONFIG['cell_path'] = 'data_x/perov_5_raw_train_cell_mat.pt'
CONFIG['data_y_path'] = "data_x/perov_5/raw_train/targets.csv"

CONFIG['unprocessed_path'] = 'data_x/perov_5_raw_train_unprocessed.txt'
...
class LinearAttention(nn.Module):
    def __init__(self, dim, heads = 2, dim_head = 4): # def __init__(self, dim, heads = 4, dim_head = 32):
        super().__init__()
...
LR = 0.0001
VERBOSE = True
device = torch_device_select(CONFIG['gpu'])
...
for epoch in range(5000):
    ...
    for batch in train_loader:
        ...
        #loss = kld
        loss = torch.nn.functional.mse_loss(noise, predicted_noise) # can we add these losses together? makes sense.

        mean_epoch_loss.append(loss.item())
        loss.backward() # should delete the loss
        optimizer.step()

    # what about model.eval? When we activatete this then it's better.
    # zero grad 
    ...
    for batch in test_loader:
        ...
        # In the batch, manually delete tensors.
        z , z_mu, z_var = unet(batch_noisy, t)
        predicted_noise = z
        ...
        #kld = torch.mean(-0.5 * torch.sum(1 + z_var - z_mu ** 2 - z_var.exp(), dim = 1), dim = 0)
        #loss = kld
        test_loss = torch.nn.functional.mse_loss(noise, predicted_noise)

        if best_test_loss > test_loss: 
                best_epoch = epoch
                best_test_loss = test_loss
                
                # check here. Try and comment these out.
                model1_best = copy.deepcopy(diffusion_model)
                model2_best = copy.deepcopy(unet)

                # Save the model such that it can be loaded further at line 496.
                torch.save(model1_best, CONFIG['model_path1']) # saving to the file 'model_path1'
                torch.save(model2_best, CONFIG['model_path2'])

                del model1_best, model2_best

        mean_epoch_loss_val.append(test_loss.item()) # not saving the whole comp. graph, only the scalers.
        
        # try these.
        del test_loss, batch, predicted_noise, batch_noisy, z , z_mu, z_var
...
print('game_on generating in 4-3-2-1')
...
with torch.no_grad():
    ...
    for k in range(100):
        ...
        for i in reversed(range(diffusion_model.timesteps)):
            ...
            if i == 0 :
                print("Values of k: {} and i: {}.".format(k, i))
                data_x, composition_mat = img[:, 0:607], img[:,607:]
                data_x = scaler.inverse_transform(data_x.cpu().data.numpy())
                data_x = torch.tensor(data_x, dtype=torch.float, device=device)
                
                data_dic['index'] = k
                data_dic['data_x'] = data_x
                data_dic['composition_mat'] = composition_mat
...
f = open("./pkl_diff_files/" + now + ".pkl", 'wb')
